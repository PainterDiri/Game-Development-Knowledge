...
这张表是课程的教学承诺与抽检入口，不是学习者成绩，也不表示本轮逐句重写了全部章节。

| 核心能力与前置 | 正文落点 | 不看答案的检验 | 证据能说明什么、不能说明什么 |
|---|---|---|---|
| 路径、进程与命令；只需会打开终端 | 第 1 章 | 解释命令运行目录、参数和退出码，定位一次路径错误 | 小例子验证当前机器的行为，不代表所有 shell 相同 |
| Git 状态与历史；依赖文件/终端 | 第 2–4 章 | 为已共享的错误提交选择恢复方式，解释哪些引用改变 | 临时仓库操作与提交图，不以背命令替代判断 |
| 协作与交付；依赖提交/分支 | 第 5、9 章 | 为失败发布选定可追溯产物和回滚路径 | 约束下的方案评估，不冒充真实团队演练 |
| 构建、环境与随机性；依赖进程和版本 | 第 6–7 章 | 改变一个输入解释输出差异，区分字节相同与行为可复现 | 主实践可比对种子、源码哈希和输出，不证明完整供应链可信 |
| 测试、诊断与安全清理；依赖构建边界 | 第 8 章，尤其 8.12；主实践 | 拒绝源目录和未知文件，保留哨兵文件，并修复一条失败回归 | `tests/test_build.py` 验证本地误操作边界，不证明原子发布或对抗并发攻击 |

第 1–5 章补全可连续执行的临时 Git 情景：三个文件版本、两个工作仓库与裸远端、分叉拒绝、merge/rebase 冲突与放弃、rescue 引用、cherry-pick/revert、bisect 及同一份候选字节的发布演示；每一步同时检查引用和实际内容。第 6–9 章已有构建、冷构建和清理保护回归继续保留。

维护者可在仓库根目录运行 `python3 scripts/test_lesson_examples.py`，直接抽取第 2–5 章的五个命令块，在新临时目录和隔离的 Git 配置中连贯执行；`make check` 已包含此入口。学习者只需按章逐条操作，不需要维护额外日志或再完成第二个主实践。

2026-09-07 已在 arm64 macOS、Git 2.55.0、Python 3.9.6 运行情景；课程建议的 Python 3.11+ 环境不因此下调。验证只使用本地传输，不证明托管服务的权限、PR 审批、分支保护、网络故障或真实商店发布；教学 tar 不是游戏构建。正文与题目由同一执行者依次切换角色走查，没有独立学习者反馈或新增跨平台矩阵结果。
...
- [主实践：可复现 seeded-room 工程](practice.md)：优先下载 ZIP 到仓库外，在 `workspace/repro-game/` 完成 Git 基础里程碑、确定性、冷构建和 bisect；已克隆仓库时才把 `.practice/toolchain-and-git/` 作为备选；
- 每章结尾均有按知识密度设计的可判定练习，题型覆盖命令实作、提交图推演、数据分析、故障诊断和发布设计；题后只保留完整“讲解、判定与验证”，不生成提示块；
