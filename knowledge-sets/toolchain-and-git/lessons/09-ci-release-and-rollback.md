...
**题型**：发布记录审计与兼容性判定
**作答产物**：补全表格；判断能否回滚；列出阻断项与验证命令。

发布系统现有记录：

| release | commit | platform | toolchain | content/schema | artifact hash | tests | previous |
|---|---|---|---|---|---|---|---|
| 1.4.1 | `a1b2` | win-x64 | Unity 2023 LTS | content 27 / save 4 | `sha256:AAA` | unit+smoke passed | 1.4.0 |
| 1.4.2 | `c3d4` | win-x64 | ? | content 28 / save 5 | ? | CI #812 passed | 1.4.1 |

1. 指出 1.4.2 至少缺哪些可审计字段；
2. 玩家使用 1.4.2 后已写出 save schema 5，而 1.4.1 只读到 schema 4。能否直接把下载渠道切回 `sha256:AAA`？
3. 设计 1.4.2 的发布前验收与两种恢复策略：复用旧 artifact、发布新的前向修复。

<details><summary>讲解、判定与验证</summary>

1.4.2 至少缺精确 Unity/平台 SDK/依赖版本、不可变 artifact 位置与 hash、测试报告身份及其实际测试的 artifact hash、构建时间/流水线 ID、签名或封装派生关系、兼容矩阵和明确回滚目标。“CI #812 passed”若未绑定同一产物，只是模糊状态。

不能直接切回 `sha256:AAA`：旧客户端不认识 schema 5，可能拒绝加载、覆盖或损坏玩家进度。先在副本数据上验证向后读取、迁移/降级策略和服务器协议兼容；若无安全降级，应阻断代码回滚，改用向前修复或提供经过测试的数据转换。永远先备份真实存档，不能拿玩家唯一副本做实验。

复用旧 artifact 的策略：确认其签名、渠道、服务端协议和数据格式仍兼容，验证 hash=`AAA`，在隔离账户做安装/启动/加载/联网冒烟，再切换渠道并监控。前向修复策略：从当前兼容基线创建新提交，构建一个新 artifact，执行单测、schema 4→5/5 保持测试、目标平台冒烟与服务端协议测试，把全部证据绑定新 hash 后发布。

评分点：记录同时回答源码身份、环境、产物身份、测试身份和兼容性；回滚不是 `git reset`；两种恢复都要求实际 artifact 与数据验证。常见错误是重新构建 1.4.1 却沿用旧测试证据，或只验证启动不验证存档/服务端。游戏映射：客户端、内容包、服务器和持久化数据独立演进，回滚任何一层前都要检查相邻协议。
</details>

### T09-Q2：cache、artifact 和 release 不能互换

**题型**：流水线设计与信任边界
**作答产物**：cache→artifact→release 数据流、权限边界和哈希验证。
