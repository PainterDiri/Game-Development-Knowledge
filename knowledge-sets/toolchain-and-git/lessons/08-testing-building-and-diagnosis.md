...
### T08-Q3：用数据判断 flaky 还是回归

**题型**：小样本性能数据分析与门禁设计
**作答产物**：两组统计量、初步结论、下一轮控制变量和门禁规则。

固定关卡与 seed 后，单位为毫秒的冷启动测量如下，阈值暂定 105 ms：

```text
baseline: 98, 101, 99, 100, 102, 97, 101, 99, 100, 98
candidate: 99, 101, 100, 162, 98, 100, 99, 158, 101, 100
```

1. 分别计算中位数、最大值和阈值失败率；
2. 解释为什么“重试三次取一次成功”和“只比较中位数”都不足以修复门禁；
3. 给出下一轮至少四个要固定/记录的变量，以及一个同时检查功能正确性与性能退化的门禁方案。

<details><summary>讲解、判定与验证</summary>

baseline 排序后中位数为 `(99+100)/2 = 99.5 ms`，最大 102 ms，失败率 0/10。candidate 排序为 `98,99,99,100,100,100,101,101,158,162`，中位数 100 ms，最大 162 ms，失败率 2/10=20%。中位数接近不能否定候选版本新增的长尾；样本也不足以断言根因，结论应是“存在可复现的候选长尾信号，需要受控复测和诊断”。

重试取一次成功把失败概率藏起来，使真实卡顿在 CI 中被放行；只看中位数则忽略玩家可感知的尾延迟。下一轮固定/记录至少包括 CPU/GPU 与电源模式、构建类型和编译选项、seed/输入规模、并发后台任务、缓存冷/热状态、运行时/驱动版本、温度与测量方法。先分别测冷/热多轮，保留原始样本与日志，再用 profiler/trace 对两次尖峰定位 I/O、锁竞争或资源导入。

一个最小门禁：功能测试和固定 seed 输出必须全部通过；性能在固定环境重复 N 次，报告 median 与 p95/最大值，并要求失败率低于预先约定值且候选相对基线的稳健统计量不超预算。阈值和 N 必须由真实预算与噪声基线决定，不能由这 10 次数据随意“调到能过”。评分点：三个数值正确；明确长尾与根因未知；提出控制变量、原始证据和功能门禁。游戏映射：平均帧时正常但偶发 160 ms 仍会形成明显卡顿，尤其影响动作游戏输入响应和联网超时。
</details>

### T08-Q4：拒绝不等于安全，必须检查没有副作用

**题型**：性质测试与失败原子性
**作答产物**：测试步骤、失败前后状态比较和边界用例。
