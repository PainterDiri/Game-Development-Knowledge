...
### T07-Q1：同提交为何 manifest 不同

相同提交和 seed 的两次构建只因绝对路径、当前时间和文件遍历顺序而不同，指出修复顺序。

<details><summary>最小提示</summary>

把确定性输入与 provenance 分开。
</details>

<details><summary>讲解与验证</summary>

先使用仓库相对路径，再排序输入列表，再把当前时间从确定性摘要移入 provenance；冷构建两次比较。跨平台字节不同需声明等价目标，但不能用它掩盖可修复差异。游戏映射：seed、内容版本和工具版本必须支持复现。
</details>
...
<details><summary>最小提示</summary>
比较共享随机流的消费顺序，以及按子系统或阶段拆分随机输入的方式。
</details>
