...
2. [课程生成流程](generation-workflow.md)：研究、课程地图、逐章生成、出题人、求知者复查和发布；
3. [章末出题人规范](question-authoring.md)：按知识密度设计、删改和验收练习题；
...
- 练习题放在所依赖章节的结尾，题面写明题型、作答产物和判定条件，题后就地提供完整讲解；不生成提示；
- 不创建独立练习题页、研究笔记页或参考书目页；
- 每门课只设计一个与该课直接相关的主实践，不要求逐课接入最终项目；
- 公开代码只读，个人修改默认进入网站 ZIP 解压后的仓库外工作区；已克隆仓库时才使用被 Git 忽略的 `.practice/<course-slug>/`；
