...
- **D3 深入掌握**：能推导/解释机制、实现简化版、测量成本、处理失败并迁移；核心知识单元按能力覆盖拆分，不以固定章数代替深度；
...
路线已规划主要方向，但专业覆盖仍需逐课用能力与证据表核对；课程内容并未全部发布：截至 **2026-09-06**，27 门课程中工具链与 Git、C 程序设计已经发布，其余 25 门仍处于准备中。不要把课程总览、课程索引或自动检查通过误认为已经学完全部知识。

- 学习基础纵切片时，优先沿“核心主线”推进；支撑课程按项目问题插入，方向拓展不阻塞单机主线；
- 只有收到生成请求时，才按索引 `order` 一门一门推进；当前下一门待生成的是离散数学。本轮只修订既有内容，不启动下一门课。路线分层只改变学习路径，不跳过课程自身的质量门禁。
- 在线服务不阻塞单机纵切片，但对多人/服务器方向是必修能力：实时同步与持久化一致性是两类问题，不能用一个联机演示代替两者。后续生成时按[教学覆盖与证据规范](../standards/teaching-evidence.md)落实，当前不扩写骨架。
