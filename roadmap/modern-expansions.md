...
### 5. 机器学习、LLM 与 Transformer

AI 不再只作为“游戏 AI”末尾的一节，而是拆成三层课程：机器学习基础、LLM/Transformer 原理、生成式 AI 与游戏工具工程。学习顺序固定为：线性代数/概率/优化 → 数据与模型 → 注意力与 Transformer → 推理/检索/工具调用 → 游戏应用与生产边界。

每一层都必须有可解释基线、数据切分、指标、失败样本和资源预算。课程要明确区分规则式 NPC、传统机器学习、深度学习和生成式模型；调用 API、展示一次漂亮生成结果或训练 loss 下降，都不能单独作为“理解 AI”的证据。

### 6. AI 辅助开发与工具智能化

可以用 AI 生成样板、测试、工具代码、资产变体和文档，但必须保留：来源、许可证、人工评审、可复现输入、测试、性能基线、隐私/安全检查、缓存和失败回退。学习重点是让 AI 成为可验证的生产工具，而不是把未经理解的代码提交进核心规则。
