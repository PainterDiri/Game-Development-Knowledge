...
实践不按每门课重新做一个“课程作业”设计，而是分成三个可复用层：

- **微实验**：30–120 分钟，验证一个原理，代码短、结果可测。
- **系统实验**：1–2 周，把多个概念组成一个工具或运行时模块。
- **主项目**：持续演进的“小以撒”纵切片；每次只增加一个系统，并保留可运行版本。

每个公开实践必须提供：`labs/README.md`（问题、约束、运行方式、验收标准）与折叠的 `labs/solutions.md`（提示、参考路线、验证、常见失败和替代方案）。个人实现放在学习者惯用位置；若在本仓库中工作，按需使用被忽略的 `.practice/<course-slug>/`。只有经脱敏、补齐许可证和复现说明的通用参考实现才进入公开 `code/`。不强制撰写实验报告或决策日志。
